...
class BPR(torch.nn.Module):
    ...
    def forward(self, user_embedding, item_embedding, u, i, j, model_ego=None):
        user = user_embedding[u]
        pos_item = item_embedding[i]
        neg_item = item_embedding[j]
        # print(torch.isnan(user).sum())
        ...
        reg_loss = model_ego.calculate_reg_loss(u, i, j)

        # print(reg_loss)
        # reg_loss = ((user ** 2).sum(dim=-1) + (pos_item ** 2 + neg_item ** 2).sum(dim=-1)).mean()
        # print(bpr_loss, reg_loss)
        # exit()
        return bpr_loss, reg_loss


    # def forward(self, user_embedding, item_embedding, u, i, j, user_embedding_ego=None, item_embedding_ego=None, gcn_feature_ego=None):
    #     user = user_embedding[u]
    #     pos_item = item_embedding[i]
    #     neg_item = item_embedding[j]
    #     # print(torch.isnan(user).sum())
    ...
    #                           negEmb0.norm(2).pow(2) + gcn_feature_ego.norm(2).pow(2)) / float(len(u))
    #     # print(bpr_loss, reg_loss)
    #     return bpr_loss, reg_loss
